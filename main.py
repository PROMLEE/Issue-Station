...
class MainWindow(QMainWindow):
    ...
    def setUserWidget(self):
        if self.user_widget_hidden :
            ...
            self.ui.user_widget.hide()
            self.user_widget_hidden = True
    ## Function for setting project list screen
    ...
    ## proj detail 화면으로
    def go_to_projdetail(self,id,name):
        self.detail = ProjDetailScreen(id,name,self)
        self.detail.exec()
    ...
    def setDataTable(self):
        ...
        for row in range(len(proj_list)) :
            ...
            self.button.clicked.connect(lambda _, id=proj_list[row]['projectid'], name=proj_list[row]['name']: self.go_to_projdetail(id,name))
